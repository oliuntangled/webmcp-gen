  ...
  if (type.isObject() && !type.isArray()) {
    ...
    for (const prop of type.getProperties()) {
      ...
      if (decl && Node.isPropertySignature(decl)) {
        ...
        const location = prop.getValueDeclaration() ?? declarations[0];
...
function generateHandlerStub(
  ...
  const hasUntrusted = !!def.annotations?.untrustedContentHint;
  ...
  if (hasUntrusted) {
    securityLines.push(
      "    // SECURITY: This tool handles untrusted/user-generated content.",
      "    // Validate, sanitise, and escape all output before rendering or storing."
    );
  }
  if (!isReadOnly) {
    securityLines.push(
  ...
  const safeDesc = escapeJsDocComment(def.description);
  const safeNameStr = escapeString(def.name);

  return `/**
 * WebMCP Tool: ${escapeJsDocComment(def.name)}
 * ${safeDesc}
 *
...
${securityBlock}    // TODO: Implement ${safeNameStr} logic here
    throw new Error("Not implemented: ${safeNameStr}");
  },
});
`;
}
...
function escapeJsDocComment(s: string): string {
  return s.replace(/\*\//g, "* /");
}
